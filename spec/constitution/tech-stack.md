...
- **Esa excepción es un caso de uso aparte, no una bandera dentro de la gestión del recetario.** `GestionDeRecetas` sabe solo de autoría; retirar vive en `RetirarPorModeracion` (020). Comparten la ruta `DELETE /recetas/{id}/publicacion` porque para quien pulsa son la misma acción, pero la retirada **avisa por correo al autor** y despublicar lo propio no. Mientras significaron lo mismo, un parámetro `esResponsable` bastaba; en cuanto dejaron de significarlo, ese parámetro era una bandera que podía colarse por descuido en editar o borrar.
