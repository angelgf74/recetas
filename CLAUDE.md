...
Endpoints: `GET /salud`, `POST /registro/solicitudes`, `POST /registro/completar`, `POST /contrasena/solicitudes`, `POST /contrasena/restablecer`, `POST /sesiones`, `GET /yo`, `PUT /yo/contrasena`, `GET|POST /recetas`, `GET /recetas/busqueda` (`?nombre=`, `?ingrediente=` repetible, `?etiqueta=` repetible, `?tipo=`), `GET /recetas/favoritas`, `GET|PUT|DELETE /recetas/{id}`, `POST|DELETE /recetas/{id}/publicacion`, `PUT|DELETE /recetas/{id}/favorito`, `POST /recetas/importaciones`, `POST /recetas/{id}/fotos`, `GET|DELETE /recetas/{id}/fotos/{fotoId}`, `PUT /recetas/{id}/fotos/{fotoId}/portada` y `GET /recetas/{id}/fotos/{fotoId}/miniatura` (todos protegidos salvo salud, alta, recuperación de contraseña e inicio de sesión).
...
**Importar también descarga la foto (027), con el mismo `DescargadorDePaginasSeguro`.** `DescargarImagenAsync` no es un cliente nuevo: comparte `HttpClient` y `ConnectCallback` con `DescargarAsync` a través de un núcleo común (`DescargarBytesAsync`), precisamente para no reabrir el hueco que la 011 cerró. La imagen se detecta por bytes y se limpia de EXIF con el mismo `ILimpiadorDeImagenes` que las fotos subidas por el usuario — una foto de un tercero puede llevar la ubicación de quien la hizo, no de quien importa. **Probar esto es más difícil de lo que parece:** contra `127.0.0.1` fijo, un cliente sin protección y uno protegido dan el mismo resultado si no hay nada escuchando ahí. El test que de verdad distingue levanta un servidor real en bucle local y comprueba que nunca se le habla.
...
**Foto de portada (009, elección explícita desde la 025):** `Receta.FotoDePortada` es la elegida por el autor (`FotoDePortadaElegidaId`) si sigue existiendo, y si no la más antigua — sigue siendo derivada para quien no elige nada. La columna que guarda la elección tiene FK a `fotos.Id` con `ON DELETE SET NULL`, una referencia cruzada a propósito entre `recetas` y `fotos`; el dominio además limpia esa misma columna en `QuitarFoto`. Las dos redes son independientes: quitar cualquiera de las dos por separado no rompe el comportamiento observable, solo deja el agregado con una referencia colgada por dentro que ningún consumidor externo llega a notar.
...
- **Las entidades con clave `Guid` generada por el dominio necesitan `ValueGeneratedNever()`** si se cuelgan de un padre ya rastreado. Si no, EF ve la clave rellena, deduce que la fila existe y emite `UPDATE` en lugar de `INSERT`; el error es un `DbUpdateConcurrencyException` de "0 filas afectadas" que no menciona la causa. A las que se añaden con `Add()` no les pasa, porque EF marca el grafo entero como nuevo. Es la razón por la que `IngredienteDeReceta` y `EtiquetaDeReceta` (024) son entidades de vínculo explícitas con clave compuesta, y no un many-to-many implícito de EF: un many-to-many reintroduciría este mismo terreno para una relación que no lo necesita.
